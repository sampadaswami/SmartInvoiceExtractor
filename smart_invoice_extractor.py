...
def extract_text_from_pdf(path):
    ...
    # Fallback to OCR if PDF text is weak
...
def auto_extract_fields(text):
    ...
    m = re.search(r"Amount\s*in\s*Words\s*[:\-]?\s*(.+)", text, re.I)
    if m:
        data["Amount In Words"] = m.group(1).strip()
...
st.caption("Automatically reads invoices • Creates columns on its own")

# 🔔 OCR DISCLAIMER (ADDED — NO OTHER CHANGE)
st.warning(
    "⚠️ The application uses automated OCR technology. Accuracy is not guaranteed. "
    "Users are responsible for verifying extracted invoice information."
)
st.caption("OCR output may not be fully accurate. Please verify before use.")
